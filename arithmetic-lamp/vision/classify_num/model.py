...
def model(x, keep_prob):
    ...
    x_image = tf.reshape(x, [-1, w, h, 3])
 
    # Conv1
    with tf.name_scope('conv1'):
        W_conv1 = weight_variable([3, 3, 3, 16], name="weight")
    ...
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([11*16*32, 256], name="weight")
        b_fc1 = bias_variable([256], name='bias')
        h_pool2_flat = tf.reshape(h_pool2, [-1, 11*16*32])
        h_fc1 = tf.nn.relu(
    ...
    with tf.name_scope('fc2'):
        W_fc2 = weight_variable([256, n_class], name="weight")
        b_fc2 = bias_variable([n_class], name='bias')
        y = tf.nn.softmax(
